defmodule DmCasesTest do
  ...
  @tag :skip
  test "Network Partition" do
    Enum.each(study_cases(), fn {node_module, %{crdt_cases: crdt_cases, conf_cases: conf_cases}} ->
      Enum.each(conf_cases, fn {conf_name, node_conf} ->
        Enum.each(crdt_cases, fn {crdt_data_type, crdt_module} ->
          Enum.each(topology_cases(), fn {topology_name, n_nodes, topology_setup_func, topology_teardown_fun} ->

            Logger.debug("Starting simulation(Network Partition): Topology=#{topology_name}, Node=#{node_module |> Module.split() |> List.last()}, Conf=#{conf_name}, CRDT=#{crdt_module |> Module.split() |> List.last()}")
            ...
            :timer.sleep(100)
            # update on the isolated node
            SimulationUtility.perform_update_on_nodes(1, SimulationUtility.repetitions(), crdt_data_type, crdt_module, :also_immediately, 20, n_nodes, 1000)
            ...
            module_name = node_module |> Module.split() |> List.last()
            crdt_name = crdt_module |> Module.split() |> List.last()
            filename = "[Network Partition] | #{topology_name} | #{module_name} | #{conf_name} | #{Atom.to_string(:also_immediately)} | #{crdt_data_type}-#{crdt_name}.json"
            SimulationUtility.extract_and_save_metrics(n_nodes+1, metrics_dir, filename)
